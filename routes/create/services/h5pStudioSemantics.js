...
export function normalizeStudioParameters(library, parameters, libraries, trustedMedia = new Map()) {
  let nodes = 0;
  const inspect = (field, input, location, depth) => {
    ...
    if (field.type === 'group') {
      ...
      for (const child of field.fields || []) {
        ...
        const normalized = inspect(child, value[child.name], `${location}.${child.name}`, depth + 1);
      ...
      if (field.name && !Object.keys(result).length && (field.fields || []).every(child => child.optional)) return undefined;
      return result;
    }
    if (field.type === 'list') {
    ...
    if (field.type === 'library') {
      ...
      return {
        ...
        params: inspect({ type: 'group', fields: entry.semantics }, value.params, `${location}.params`, depth + 1),
  ...
  return inspect({ type: 'group', fields: entry.semantics }, parameters, 'params', 0);
}
...
export function buildStudioContract(library, libraries, templateParameters) {
  ...
  const visit = (name, depth = 0) => {
    ...
    const compact = field => {
      const out = {};
      for (const key of ['name', 'type', 'label', 'description', 'default', 'optional', 'min', 'max', 'maxLength', 'multiple', 'regexp']) {
...
export function buildStudioJSONContract(library, libraries, templateParameters, trustedMedia = new Map()) {
  ...
  const convert = field => {
    let schema;
    if (field.type === 'group') schema = group(field.fields);
      ...
      const selection = { enum: (field.options || []).flatMap(option => option.options || [option]).map(option => option.value) };
...
export function collectTemplateMedia(library, parameters, libraries, contentId) {
  const files = new Map();
  const walk = (field, value, depth = 0) => {
    ...
    if (media.has(field.type)) {
      ...
      for (const child of field.fields || []) walk(child, value[child.name], depth + 1);
      ...
      walk({ type: 'group', fields: libraries.get(value.library).semantics }, value.params, depth + 1);
    }
  };
  walk({ type: 'group', fields: libraries.get(library)?.semantics }, parameters);
  return files;
}
...
export function validateStudioMediaTemplate(library, parameters, catalog) {
  ...
  if (type?.category !== 'Media activities') return;
